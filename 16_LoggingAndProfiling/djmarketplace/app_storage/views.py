...
class CartView(View):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            bonus_account.save()
            logger.info('Bonuses are writen-off')
            cart.delete()
            ...
            # Если статус не максимальый, то вычисляем, какой статус нужно назначить
                ...
                if total_check > 500 and total_check <= 2000:
                    ...
                    bonus_account.save()
                    logger.info('New loyalty status: Friend')
                # Если сумма покупок выше 2000$, то назначаем статус Best friend
                    ...
                    bonus_account.save()
                    logger.info('New loyalty status: Best friend')
